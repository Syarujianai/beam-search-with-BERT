...
def gen_embeddings(vocab):
    ...
    if(os.path.exists(config.save_path_dataset+'embeddings.p')):
        with open(config.save_path_dataset+'embeddings.p', "rb") as f:
            embeddings = pickle.load(f)
        print("last embeddings LOADED FROM PICKLE")
    else:
        if config.emb_file is not None:
            ...
            # save generated embedding (correspond to LCQMC vocab)
            if not os.path.exists(config.save_path):
                os.makedirs(config.save_path)
            with open(config.save_path+'embeddings.p', "wb") as f:
                pickle.dump(embeddings, f)
                print("Saved last embeddings")
...
def share_embedding(vocab, pretrain=True):
    if config.read_renmin:
        embedding = nn.Embedding(4801, config.emb_dim)
    else:
    ...
    for j, batch in pbar:
        ...
        if((j<3 and ty != "test") or ty =="test"): 
            ...
            for i in range(len(batch["target_txt"])):
                ...
                ref.append(" ".join(list(batch["target_txt"][i])))
                dial.append(" ".join(list(batch['input_txt'][i])))
